...
# Create Dirichlet distributions for each pixel
alpha = np.ones(10)  # Equal concentration parameters for simplicity
dirichlet_samples = np.random.dirichlet(alpha, size=(pixel_size, pixel_size))
...
def calculate_component_probabilities():
    ...
    # Get the Dirichlet sample for this point
    joint_probs = dirichlet_samples[x_coord, y_coord]
...
# Function to select digits of each pixel using probs as weight
def select_digit(x, y):
    joint_probs = dirichlet_samples[x, y]
...
# Allocate each client one label
for i in range(client_num):
    point = clients[i]
...
# Allocate 100 datasets for each client
clients_datasets = []
